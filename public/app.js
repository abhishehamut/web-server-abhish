...
form.addEventListener('submit', async (event) => {
  ...
  item.innerHTML = `<span class="entry-display"><strong>${saved.title}:</strong> ${saved.body}</span><button class="favorite-btn" type="button" aria-label="Add to favorites">${saved.favorite ? '★' : '☆'}</button><button class="edit-btn hover:bg-teal-50" type="button">Edit</button><button class="delete-btn hover:bg-red-50" hx-delete="/entries/${saved.id}" hx-target="closest li" hx-swap="outerHTML" hx-confirm="Delete this entry?" hx-indicator="next .delete-indicator">Delete</button><span class="delete-indicator htmx-indicator">Deleting...</span>`;
  list.append(item);
  htmx.process(item);
